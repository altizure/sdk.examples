...
__新功能:__
* 新增 `AltizureProjectMarker` 支持 `opacity` 透明度接口。

__缺陷修复:__
* 使用 `IE Edge` 浏览器无法加载实景模型。
* 使用 `IE 11` 无法加载 `OSG` 模型。

__改进:__
* 优化点云加载，减少卡顿。
* 即使没有使用 `camera::pose`，sdk2 也会发出升级警告。
...
__新功能:__
* 添加 AltizureProjectMarker::opacity 接口。在 texture 渲染模式下，可调整三维模型透明度。
...
__新功能:__
* 新增 `OSGMarker` 以支持 osg/osgb 格式 LOD 数据。

__缺陷修复:__
* 一体机上星球加载卫星图报错/显示错误。

__改进:__
* 优化 `AltizureProjectMarker::pickDepthMap`，拾取时间从 ~1000ms 减少至 >50ms。
* 增加升级至 `sdk3` 的提示和指南。
* sdk 全名更改为 `Altizure Javascript 3D SDK`。
* 增加持续集成管线。
...
__缺陷修复:__
* `Marker::Euler` 和 `Marker::orientation` 同时使用时，可能出现的更新无效。
* 显存 over-estimated 导致的场景加载不稳定。
...
__缺陷修复:__
* 修复IE11 Uint8Array 兼容性问题。

__改进:__
* DEM、photolod 数据的显存管理。
